...
import com.lms2ue1.sbsweb.dto.StatusCreationDto;
...
public class StatusTransitionController {
    ...
    public String updateStatus(@PathVariable long sID, @ModelAttribute StatusCreationDto form,
	    BindingResult bindingResult, Principal principal, Model model) throws AuthenticationException {
	String username = principal.getName();
	if (!auth.isSysAdmin(username)) {
	    return "error";
	} else if (bindingResult.hasErrors()) {

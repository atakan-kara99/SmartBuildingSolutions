...
public class ContractController {
    ...
    public String showContractDetails(@AuthenticationPrincipal User user, @PathVariable Long pID,
...
//	model.addAttribute("billingItems", billingItems.stream().filter(billingItem -> billingItem.getCID() == cID).collect(Collectors.toList()));
	...
	model.addAttribute("billingItems", List.of(new BillingItem("Heizung montieren", 0L), new BillingItem("Fenster einbauen", 1L)));
